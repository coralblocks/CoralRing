...
/**
 * <p>
 * The implementation of a non-waiting {@link RingConsumer}. Of course it will wait only if the ring becomes empty, in other words, if the producer
 ...
 * </p>
 * <p>
 * If the consumer is too slow and falls behind, its {@link NonWaitingRingConsumer#availableToFetch()} method will return <code>-1</code> to indicate that
 ...
 */
public class NonWaitingRingConsumer<E extends MemorySerializable> implements RingConsumer<E> {
	
	private static final int DEFAULT_CAPACITY = NonWaitingRingProducer.DEFAULT_CAPACITY;
	
	private static final int SEQ_PREFIX_PADDING = NonWaitingRingProducer.SEQ_PREFIX_PADDING;

	private static final int HEADER_SIZE = NonWaitingRingProducer.HEADER_SIZE;
	
	private static final int CHECKSUM_LENGTH = NonWaitingRingProducer.CHECKSUM_LENGTH;
	 ...
	 * @param filename the file to be used by its shared memory
	 */
	public NonWaitingRingConsumer(int capacity, int maxObjectSize, Class<E> klass, String filename) {
	 ...
	 * @param checkChecksum true to check the checksum in order to avoid reading a corrupt message in the situation where the consumer trips over the producer
	 */
	public NonWaitingRingConsumer(int capacity, int maxObjectSize, Class<E> klass, String filename, boolean checkChecksum) {
	 ...
	 * @param fallBehindTolerance the percentage of the capacity that the consumer can fall behind before giving up in order to avoid getting too close to the edge
	 */
	public NonWaitingRingConsumer(int capacity, int maxObjectSize, Class<E> klass, String filename, boolean checkChecksum, float fallBehindTolerance) {
	 ...
	 * @param fallBehindTolerance the percentage of the capacity that the consumer can fall behind before giving up in order to avoid getting too close to the edge
	 */
	public NonWaitingRingConsumer(int capacity, int maxObjectSize, Class<E> klass, String filename, float fallBehindTolerance) {
		this(capacity, maxObjectSize, Builder.createBuilder(klass), filename, DEFAULT_CHECK_CHECKSUM, fallBehindTolerance);
	}
	
	/**
	 * <p>Creates a new non-waiting ring consumer with the default capacity (i.e. 1024).</p>
	 * 
	 * @param maxObjectSize the max size of a single message
	 * @param builder the builder producing new instances of the message
	 * @param filename the file to be used by its shared memory
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Builder<E> builder, String filename) {
	 ...
	 * @param checkChecksum true to check the checksum in order to avoid reading a corrupt message in the situation where the consumer trips over the producer
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Builder<E> builder, String filename, boolean checkChecksum) {
	 ...
	 * @param fallBehindTolerance the percentage of the capacity that the consumer can fall behind before giving up in order to avoid getting too close to the edge
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Builder<E> builder, String filename, boolean checkChecksum, float fallBehindTolerance) {
	 ...
	 * @param fallBehindTolerance the percentage of the capacity that the consumer can fall behind before giving up in order to avoid getting too close to the edge
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Builder<E> builder, String filename, float fallBehindTolerance) {
	 ...
	 * @param filename the file to be used by its shared memory
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Class<E> klass, String filename) {
	 ...
	 * @param checkChecksum true to check the checksum in order to avoid reading a corrupt message in the situation where the consumer trips over the producer
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Class<E> klass, String filename, boolean checkChecksum) {
	 ...
	 * @param fallBehindTolerance the percentage of the capacity that the consumer can fall behind before giving up in order to avoid getting too close to the edge
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Class<E> klass, String filename, boolean checkChecksum, float fallBehindTolerance) {
	 ...
	 * @param fallBehindTolerance the percentage of the capacity that the consumer can fall behind before giving up in order to avoid getting too close to the edge
	 */
	public NonWaitingRingConsumer(int maxObjectSize, Class<E> klass, String filename, float fallBehindTolerance) {
